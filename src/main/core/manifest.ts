...
- **system** — see/control the Mac: battery, volume, brightness, displays, Wi-Fi, apps, notify, clipboard, caffeinate, lock/sleep/screenshot · Mac status & control · one op per call; app_quit/lock/sleep are T2; some ops need macOS TCC permission · docs/tools/system.md
...
- **memory** — file-based long-term memory: read/append/remember/recall/list/note/handoff · persist facts+events, recall the past, capture notes when a task ends · never invent memories; never edit the stable layer · docs/tools/memory.md
