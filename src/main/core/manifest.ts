...
- **team** — manage the specialist agent ROSTER (create/list/delete): named agents that EXTEND the fixed three, each with its OWN model + a private knowledge folder + a grant + a privilege role · build a specialist ("a Coder on claude-opus-4-8") · create/delete are T2, list is T0; model can be ANY catalog id; grant is the agent's autonomy allowlist (default read+notify); delegationRole is the PRIVILEGE role leaf (default: no spawn/schedule/vault/message-user) | orchestrator (may spawn children, bounded); dailyTokenBudget is an optional per-agent daily token cap for autonomous runs (delegate/study, omitted → unlimited beyond the global kill-switch); only persists+scaffolds (agents/<id>/knowledge/ + shared agents/index.md), RUN one with delegate_to_agent · docs/tools/team.md
