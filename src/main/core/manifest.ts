...
- **schedule** — create/list/pause/resume/delete/edit recurring Scheduled Jobs that persist + re-arm on boot · a live auto-refreshing widget (\`fetch\`: HTTP GET on a timer, 0 tokens) or a recurring autonomous task (\`agent\`: prompt-driven turn) · create/edit/pause/resume/delete are T2, list is T0; ASK the user the autonomy grant before an \`agent\` job (default read+notify); only persists+schedules, never runs · \`edit\` merges — send only the fields you change (omitted ones keep their current value, e.g. changing the schedule keeps a custom render.html) · docs/tools/schedule.md
