...
- **inbox** — message the **USER** asynchronously (**async HITL — NEVER blocking**) · `ask_user {kind, subject, body?, projectSlug?, cardId?, idempotencyKey?}` / `list_answers {agentId?}` · when you need a human decision to proceed, **WRITE** the ask, **checkpoint** the card (`awaiting_human` ⏳), and **YIELD** — do NOT wait; the user's answer re-wakes you and you read it with `list_answers` (the automatic resume is a later stage; for now, poll `list_answers`) · `kind` is `ask_user_questions|request_confirmation|suggest_tasks`; the user replies with a **typed action** `accept|edit|respond|reject` (reject carries a **mandatory reason** that comes back as context); `idempotencyKey` **dedupes** a retried ask; a later user **comment** on the card **supersedes** a stale pending ask (no zombie question) · **GATE (fail-closed):** only an agent allowed to message the user may `ask_user` — an **orchestrator** always, a **leaf** ONLY with `can_message_user` (else refused with a clear error) · `ask_user` is **T1**, `list_answers` is T0 · this is **SEPARATE** from the formal T0–T3 tool approvals (two-tier: approvals gate a dangerous tool CALL; the inbox is a conversation you start) · [docs/tools/inbox.md](docs/tools/inbox.md)
