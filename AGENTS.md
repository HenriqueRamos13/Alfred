...
- **schedule** — create/list/pause/resume/delete/edit recurring **Scheduled Jobs** that persist and re-arm on boot · a live auto-refreshing widget (`fetch`: HTTP GET on a timer, ZERO tokens) or a recurring autonomous task (`agent`: a prompt-driven turn, costs tokens) · create/edit/pause/resume/delete are **T2**, list is T0; before creating an `agent` job ASK the user the autonomy level (grant, default read+notify); this tool only persists+schedules, it never runs a job · `edit` **merges** (send only the fields you change; omitted fields keep their current value — so changing just the schedule preserves a custom `render.html`) · for a CUSTOM chart pass `render:{tier:2, html}` — a self-contained page you write (no network, sandboxed iframe; use the injected `Alfred.onData`/`Alfred.sparkline` runtime) · [docs/tools/schedule.md](docs/tools/schedule.md)
