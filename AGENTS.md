...
- **team** — manage the specialist agent **ROSTER** (create/list/delete): named agents that **extend** the fixed three, each with its OWN model + a private knowledge folder + a **grant** + a **privilege role** · build a specialist ("a Coder on `claude-opus-4-8`") · create/delete are **T2**, list is T0; the model can be ANY catalog id (an unknown provider/model is rejected); `grant` is the agent's autonomy allowlist when delegated to (default `["read","notify"]`); `delegationRole` is the **privilege role** (distinct from the free-text `role` specialty): **`leaf`** (default — cannot spawn/delegate, schedule, manage the roster, write the shared vault, or message the user) or **`orchestrator`** (may spawn a child via `delegate_to_agent`, bounded); `dailyTokenBudget` is an optional per-agent daily token cap for autonomous runs (delegate/study — omitted → unlimited beyond the global kill-switch); it persists + scaffolds (`agents/<id>/knowledge/` + the shared `agents/index.md`), and you **run** one with `delegate_to_agent` · [docs/tools/team.md](docs/tools/team.md)
